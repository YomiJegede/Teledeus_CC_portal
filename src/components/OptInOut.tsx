...
import { ArrowLeft, CheckCircle, XCircle, ToggleLeft, ToggleRight, Shield, Clock, User, AlertTriangle, Search, History, Download, File } from 'lucide-react';
...
const OptInOut: React.FC<OptInOutProps> = ({ onBack }) => {
  ...
  const exportToCSV = () => {
    const headers = ['Beneficiary', 'Date', 'Status'];
    const csvContent = [
      headers.join(','),
    ...
    saveAs(blob, `mca-opt-in-out-history-${new Date().toISOString().split('T')[0]}.csv`);
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">
                      {serviceStatus.isOptedIn ? 'Service Active' : 'Service Inactive'}
                    </h2>
